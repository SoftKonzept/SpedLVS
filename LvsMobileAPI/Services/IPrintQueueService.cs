...
namespace LvsMobileAPI.Services
{
    public interface IPrintQueueService
    {
        ...
        public ResponsePrintQueue POST_PrintQueue_Add(ResponsePrintQueue resPrintQueue);
    ...
    public class PrintQueueService : IPrintQueueService
    {
        ...
        public ResponsePrintQueue POST_PrintQueue_Add(ResponsePrintQueue resPrintQueue)
        {
            ...
            responsePrintQueue.Success = false;

            try
            {
                if ((resPrintQueue.SelectedAusgang is Ausgaenge) && (resPrintQueue.SelectedAusgang.Id > 0))
                {
                    PrintQueueViewData pqVD = new PrintQueueViewData(resPrintQueue.UserId);
                    responsePrintQueue = pqVD.CreatePrintInsertByAusgang(resPrintQueue);
                }
            }
            catch (Exception ex)
            {
                resPrintQueue.Success = false;
                resPrintQueue.Error = ex.Message;
            }
            return resPrintQueue;
        }
