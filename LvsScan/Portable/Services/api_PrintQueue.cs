...
namespace LvsScan.Portable.Services
{
    ...
    public class api_PrintQueue : apiSetting
    {

        public api_PrintQueue()
        {
            client = InitHttpClient();
        }
        public api_PrintQueue(Users myUser) : this()
        {
            LoggedUser = myUser.Copy();
        }
        public api_PrintQueue(enumHTTPMethodeType HttpType)
        {
        ...
        /// <summary>
        ///            /api/PrintQueue/POST_PrintQueue_Add
        /// </summary>
        public Uri Uri_POST_PrintQueue_Add
        {
            get
            {
                string tmp = ServerUrl + "/api/PrintQueue/POST_PrintQueue_Add";
                Uri retUri = new Uri(tmp);
                return retUri;
            }
        }
        public async Task<ResponsePrintQueue> POST_PrintQueue_Add(ResponsePrintQueue responsPrintQueue)
        {
            responsPrintQueue.Success = false;
            responsPrintQueue.Error = string.Empty;
            responsPrintQueue.Info = string.Empty;

            try
            {
                var json = JsonConvert.SerializeObject(responsPrintQueue);
                HttpContent httpContent = new StringContent(json);
                httpContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
                var response = await client.PostAsync(Uri_POST_PrintQueue_Add, httpContent);
                if (response.IsSuccessStatusCode)
                {
                    var jwt = await response.Content.ReadAsStringAsync();
                    var reply = JsonConvert.DeserializeObject<ResponsePrintQueue>(jwt);
                    responsPrintQueue = reply.Copy();
                }
            }
            catch (Exception ex)
            {
                string mes = ex.InnerException.Message;
                responsPrintQueue.Error = mes;
            }
            return responsPrintQueue;
        }
