...
namespace Sped4.Classes
{
    class clsFrachtKonditionen
    {
        ...
        private decimal _ID;
        private decimal _KD_ID;
        private decimal _EP;
        private decimal _km;
        private decimal _Gewicht;
        //private decimal _Gut_ID;
        private decimal _PreisTonne;
        ...
        public void InsertKDKonditionen(Int32 Konditionsart, DataTable dt)
        {
            if (dt.Rows.Count > 0)
            {
                for (Int32 i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    ...
                    Gewicht = Convert.ToDecimal(dt.Rows[i]["to"].ToString());
                    PreisTonne = Convert.ToDecimal(dt.Rows[i]["€/To"].ToString());
                    EP = Convert.ToDecimal(dt.Rows[i]["EP"].ToString().ToString());
                    PreisPal = Convert.ToDecimal(dt.Rows[i]["€/EP"].ToString());
                    km = Convert.ToDecimal(dt.Rows[i]["km"].ToString().ToString());
                    PreisKm = Convert.ToDecimal(dt.Rows[i]["€/km"].ToString());
        ...
        private void UpdateKonditionenByID(Int32 Konditionsart)
        {
            try
            {
                ...
                switch (Konditionsart)
                {
                    case 1:
                        //€/to 
                        sql = "Update Frachtkonditionen SET Gewicht='" + Gewicht.ToString().Replace(",", ".") + "', PreisTo='" + PreisTonne.ToString().Replace(",", ".") + "', km='" + km + "' WHERE ID='" + ID + "'";
                        break;
                    case 2:
                        //€/EP 
        ...
        //
        public static bool IsKDhinterlegt(decimal KD_ID)
        {
            bool IsIn = false;
            string sql = string.Empty;
        ...
        //
        // public void LoadTarife(ref frmTarifErfassung Kondition)
        public void LoadTarife()
        {
            /****
          DataTable dt = new DataTable();
          dt.Clear();
