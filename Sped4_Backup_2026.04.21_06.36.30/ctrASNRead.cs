    ...
    public partial class ctrASNRead : UserControl
    {
            ...
            if (rtsDisplayProcess.Value)
            {
                ...
                if (selectedAsnList.Count > 0)
                {
                    ...
                    foreach (var item in selectedAsnList)
                    {
                        ...
                        var listLfs = listArtItem.Where(x => !string.IsNullOrEmpty(x.LfsNr))
                                                .Select(x => x.LfsNr)
                                                .Distinct()
                                                .ToList();
        ...
        private void DoCreateEingangEdifact()
        {
            ...
            if (ListEingangToCreate.Count > 0)
            {
                ...
                //-- Gesamtanzahl aller distinct Lieferscheine über alle selektierten ASNs ermitteln
                int iTotalLfsCount = 0;
                foreach (var itm in ListEingangToCreate)
                {
                    iTotalLfsCount += itm.ListArticleInEingang
                        .Where(x => !string.IsNullOrEmpty(x.Lfs))
                        .Select(x => x.Lfs)
                        .Distinct()
                        .Count();
                }
                ...
                barVda.Minimum = 0;
                barVda.Value1 = 0;
                barVda.Maximum = iTotalLfsCount;

                stopwatch.Reset();
                stopwatch.Start();
                foreach (var item in ListEingangToCreate)
                {
                    //--  item enthält die Daten der Edi und eine liste der Artikel
                    ...
                    foreach (var lfs in listLfs)
                    {
                        ...
                        bool bReturn = asnReadViewData.CreateStoreInByAsnEdifactView(item.AsnMessage, item.eingang, listArticleForEingangCreation, (int)_ctrMenu.GL_User.User_ID);
                        if (!bReturn)
                        {
                        ...
                        barVda.Text = string.Empty;
                        lvLogEDIFACT.Items.Add(string.Empty);
