...
{
    public class ediHelper_EdiEDIFACT_ASN_D96A_CheckProcessableASN
    {
            ...
            try
            {
                if (File.Exists(myFilePathName))
                {
                       ...
                       )
                    {
                        ...
                        //if (!bVersenderOK)
                        //{
                        //    //-- dann CHeck Sender global in dem Arbeitsbereich/Mandant
                        //    bVersenderOK = dicSenderVerweis.ContainsKey(strVerweisGlobalSender);
                        //    if (!bVersenderOK)
                        //    {
                        //        //check gibt es diesen Verweis in der ganzen DB
                        ...
                        if (!myReturn)
                        {
                            ...
                            Error.Aktion = "Task_ReadASN - ediHelper_EdiEDIFACT_ASN_D96A_GetAdrVerweis ";
                            ...
                            if (AsnCheck.DicSenderVerweis.Count > 0)
                            {
                                ...
                                {
                                    strMes += iCount.ToString("000") + ":  " + item.Key + " | [ID]: " + item.Value.ID.ToString("000") + " <> " + item.Value.Verweis + Environment.NewLine;
                                    iCount++;
                                }
                            }
