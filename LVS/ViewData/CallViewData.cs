...
namespace LVS.ViewData
{
    public class CallViewData
    {
        ...
        public CallViewData(int myUserId) : this()
        {
            BenutzerID = myUserId;
        }
        public CallViewData(Calls myCall, int myUserId) : this()
        {
            Abruf = myCall.Copy();
            BenutzerID = myUserId;
        }
        public CallViewData(Globals._GL_SYSTEM myGLSystem, Globals._GL_USER myGLUser, clsSystem mySystem) : this()
        {
            GLSystem = myGLSystem;
            GL_USER = myGLUser;
            System = mySystem;
        ...
        public CallViewData(int myId, int myUserId) : this()
        {
            Abruf.Id = myId;
            BenutzerID = myUserId;
            if (Abruf.Id > 0)
        ...
        public void GetCallListOpen()
        {
            ListCallOpen = new List<Calls>();
            string strSQL = string.Empty;
            DataTable dt = new DataTable("Call");
            strSQL = sql_Get_Callopen;
            //strSQL += " and a.Aktion='Abruf'";

            dt = clsSQLcon.ExecuteSQLWithTRANSACTIONGetDataTable(strSQL, "GetCall", "OpenCall", BenutzerID);
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    SetValue(dr);
                    ListCallOpen.Add(Abruf);
                }
            }

        }

        public void GetCallbyLvsNr(int myLvsNr)
        {
            ListCallOpen = new List<Calls>();
            string strSQL = string.Empty;
            DataTable dt = new DataTable("Call");
            ...
            dt = clsSQLcon.ExecuteSQLWithTRANSACTIONGetDataTable(strSQL, "GetCallbyLvsNr", "OpenCall", BenutzerID);
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
        ...
        public string sql_Add
        {
            get
            {
                ...
                strSql = "INSERT INTO Abrufe (IsRead, ArtikelID, LVSNr, Werksnummer, Produktionsnummer, Charge, Brutto, CompanyID, CompanyName" +
                                             ...
                                             ", ASNLieferant, ASNQuantity, ASNUnit, Description, EdiDelforD97AValueId " +
                                            ") " +
        ...
        ///             GET
        /// </summary>
        public string sql_Get_Callopen
        {
            get
            {
                string strSql = string.Empty;
                strSql = "Select a.* " +
                                ...
                                "FROM Abrufe a " +
                                "INNER JOIN Arbeitsbereich b on b.Id = a.AbBereich " +
                                "WHERE a.IsRead= 0 and a.Aktion='Abruf' and IsCreated=1 " +
                                 " ORDER BY a.EintreffDatum, a.EintreffZeit ";

                return strSql;
            }
        }

        public string sql_Get_CallForStoreIn
        {
            get
            {
                string strSql = string.Empty;
        ...
        {
            sqlCreater_Call sqlCreater = new sqlCreater_Call();
            string strSql = sqlCreater.sqlString_Call_GetOpenCallById(myResCall.ListCallForStoreOut);
            ...
            if (dtCall.Rows.Count > 0)
            {
                ...
                {
                    AusgangViewData ausgangVD = new AusgangViewData(CreatedAusgang.Id, BenutzerID, true);
                    if (ausgangVD.ListArticleInAusgang.Count > 0)
                    {
                        //Common.SqlStatementCreater.sqlCreater_Article sqlArtikel = new Common.SqlStatementCreater.sqlCreater_Article();
                        strSql = string.Empty;
                        //strSql = sqlArtikel.sqlString_Article_UpdateIdentifiedByScan(ausgangVD.ListArticleInAusgang);
                        //strSql = sqlCreater_Article.sqlString_Article_UpdateIdentifiedByScan(ausgangVD.ListArticleInAusgang);
        ...
        public static bool ExecuteSQLProcessTransaction(List<string> mySqlList)
        {
            bool bReturn = false;
            string strSql = string.Empty;
            foreach (string s in mySqlList)
            {
                strSql += s;
            }
            bReturn = clsSQLcon.ExecuteSQLWithTRANSACTION(strSql, "Rebook", 1);
