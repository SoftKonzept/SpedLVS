...
{
    public class GlobalFieldVal_ArticleCountInEdi
    {
        public const string const_GlobalVar_ArticleCountInEdi = "#GlobalVar_ArticleCountInEdi#";
        ...
        public static int Check(
                         ref Dictionary<string, ASNArtFieldAssignment> myDictASNArtFieldAssignment,
                         DataTable myDt719
                         )
        {
            DataTable dt719Tmp = myDt719.DefaultView.ToTable();
            ...
            {
                if (
                        ((ASNArtFieldAssignment)itm.Value).IsGlobalFieldVar
                        &&
                        ((ASNArtFieldAssignment)itm.Value).GlobalFieldVar.Length > 0
                        &&
                        ((ASNArtFieldAssignment)itm.Value).GlobalFieldVar.Equals(GlobalFieldVal_ArticleCountInEdi.const_GlobalVar_ArticleCountInEdi)
                  )
                {
                    // do something with entry.Value or entry.Key
                    switch (itm.Key)
