...
namespace LVS
{
    public class clsFakturierung
    {
        ...
        //DB Enfernungen
        private decimal _Start_ADR_ID;
        private decimal _Ziel_ADR_ID;
        private decimal _EntfernungsID;
        ...
        private decimal _AP_ID;         //ID AuftragPos
        ...
        //manuelle RG / GS
        private decimal _zwZM;
        private decimal _zwAufliefer;
        private decimal _zwSU;
        ...
        {
            get { return _GesamtBrutto; }
            set { _GesamtBrutto = value; }
        }
        ...
        {
            //DateTime
            if (vonZeitraum.ToShortDateString() == "01.01.0001")
            {
                vonZeitraum = DateTime.MinValue.AddYears(2000);
            }
            if (bisZeitraum.ToShortDateString() == "01.01.0001")
            {
                bisZeitraum = DateTime.MaxValue.AddYears(-5000);
            }
            ...
            try
            {
                ...
                sql = "SELECT " +
                                ...
                                "(Select SUM(Artikel.Brutto) FROM Artikel WHERE Artikel.AuftragID=AuftragPos.Auftrag_ID AND Artikel.AuftragPos=AuftragPos.AuftragPos) as 'Brutto', " +
                ...
                switch (Abrechnungsliste)
                {
                    ...
                    case 3:
                        sql = sql + "WHERE AuftragPos.Status>'4' AND AuftragPos.Status<'7' AND " +
                                    ...
                                    "(SELECT Kommission.E_Zeit FROM Kommission WHERE Kommission.Auftrag=AuftragPos.Auftrag_ID AND Kommission.AuftragPos=AuftragPos.AuftragPos)<'" + bisZeitraum + "' AND " +
        ...
        //
        public DataTable GetAuftragForAbbrechnungListeByRGID(decimal RG_ID)
        {
            DataTable dataTable = new DataTable("Auftragsdaten");
            dataTable.Clear();
            if (RG_ID > 0)
        ...
        //
        //
        public static decimal GetRGEforFVGS(decimal ap_id)
        {
            decimal ADR_ID = 0;
            SqlDataAdapter ada = new SqlDataAdapter();
            SqlCommand Command = new SqlCommand();
        ...
        private void AssignValueRGGS(string Tabelle)
        {
            ...
            {
                if (ds.Tables[Tabelle].Rows[i]["AP_ID"] != null)
                {
                    AP_ID = (decimal)ds.Tables[Tabelle].Rows[i]["AP_ID"];
                }
                if (ds.Tables[Tabelle].Rows[i]["Artikel_ID"] != null)
                {
                    Artikel_ID = (decimal)ds.Tables[Tabelle].Rows[i]["Artikel_ID"];
                }
                if (ds.Tables[Tabelle].Rows[i]["Fracht"] != null)
                {
                ...
                if (ds.Tables[Tabelle].Rows[i]["km"] != null)
                {
                    km = (Int32)ds.Tables[Tabelle].Rows[i]["km"];
                }
                if (ds.Tables[Tabelle].Rows[i]["fpflGewicht"] != null)
                {
                    fpflGewicht = (decimal)ds.Tables[Tabelle].Rows[i]["fpflGewicht"];
                }
                if (ds.Tables[Tabelle].Rows[i]["Pauschal"] != null)
                {
                ...
                {
                    KundeID = (decimal)ds.Tables[Tabelle].Rows[i]["Fracht_ADR"];
                }
                else
                {
                    KundeID = (decimal)ds.Tables[Tabelle].Rows[i]["KD_ID"];
                }
                ...
                if (ds.Tables["FVGS"] != null)
                {
                    ...
                    {
                        B_ID = (decimal)ds.Tables[Tabelle].Rows[i]["FV_B_ID"];
                    }
                    if (ds.Tables[Tabelle].Rows[i]["FV_E_ID"] != DBNull.Value)
                    {
                        E_ID = (decimal)ds.Tables[Tabelle].Rows[i]["FV_E_ID"];
                    }
                }
                if (ds.Tables["GS_SU"] != null)
                {
                    GSanSU = (bool)ds.Tables[Tabelle].Rows[i]["GS_SU"];
                    SU_ID = (decimal)ds.Tables[Tabelle].Rows[i]["KD_ID"];
                }
        ...
        private void AssignValue(string Tabelle)
        {
            ...
            for (Int32 i = 0; i <= ds.Tables[Tabelle].Rows.Count - 1; i++)
            {
                ...
                if (Tabelle == "FVGSDaten")
                {
                    ...
                    {
                        B_ID = (decimal)ds.Tables[Tabelle].Rows[i]["FV_V_ID"];
                    }
                    if (ds.Tables[Tabelle].Rows[i]["FV_E_ID"] != DBNull.Value)
                    {
                        E_ID = (decimal)ds.Tables[Tabelle].Rows[i]["FV_E_ID"];
        ...
        private void InsertFracht()
        {
            ...
            try
            {
                ...
                Command.CommandText = "INSERT INTO Frachten (AP_ID, " +
                                                             "Artikel_ID, " +
                                                             "Fracht, " +
                                                             "Fracht_ADR, " +
                                                             "Frachttext, " +
        ...
        //
        public static void updateFVADR(decimal AP_ID, string ADRBezeichnung, decimal ADR_ID)
        {
            string sql = string.Empty;
            try
            {
        ...
        //
        public static DataTable GetAP_IDFromFrachten(decimal RGNummer)
        {
        ...
        //
        public static decimal GetRGNummerFromFrachten(decimal AP_ID, bool GS, bool FVGS)
        {
            decimal RGNr = 0.0m;
            string sql = string.Empty;
            SqlDataAdapter ada = new SqlDataAdapter();
        ...
        //
        public static bool IsAPisInFVGS(decimal AP_ID)
        {
            bool IsUsed = false;
            SqlDataAdapter ada = new SqlDataAdapter();
            SqlCommand Command = new SqlCommand();
        ...
        //
        public static bool IsAPisInRG(decimal AP_ID, bool RG)
        {
            bool IsUsed = false;
            SqlDataAdapter ada = new SqlDataAdapter();
            SqlCommand Command = new SqlCommand();
        ...
        //
        public static bool CheckForAbrechnungsArt(decimal AP_ID, bool GS, bool FVGS, bool GSSU)
        {
            bool IsUsed = false;
            SqlDataAdapter ada = new SqlDataAdapter();
            SqlCommand Command = new SqlCommand();
        ...
        //
        //
        public static bool IsAPinFrachtenFVGS(decimal AP_ID)
        {
            bool IsUsed = false;
            SqlDataAdapter ada = new SqlDataAdapter();
            SqlCommand Command = new SqlCommand();
        ...
        //
        public static bool IsAPisInRGSU(decimal AP_ID)
        {
            bool IsUsed = false;
            SqlDataAdapter ada = new SqlDataAdapter();
            SqlCommand Command = new SqlCommand();
        ...
        //---------- Fracht AuftragPosition bereits vorhanden -----------------
        //
        public static bool IsAPisInRGArtikel(decimal AP_ID, decimal ArtikelID, bool RG)
        {
            Int32 ibit = 0;
            if (!RG)
            {
        ...
        //------------ Read Datensatz ----------------
        //
        public DataTable GetFrachtDatenByRGID(decimal RG_ID)
        {
            string sql = string.Empty;
        ...
        public DataTable GetFrachtDaten(bool Einzelposition, string Abrechnungsart)
        {
            string sql = string.Empty;
            DataTable dataTable = new DataTable("Frachtdaten");
            dataTable.Clear();
        ...
        //
        public static void UpdateRGNrToZero(decimal RG_ID)
        {
            string sql = string.Empty;
            try
            {
        ...
        private DataSet SetDatenFromDS(DataSet ds)
        {
            if (ds.Tables["RGGSDaten"].Rows.Count > 0)
            {
                ...
                {
                    AP_ID = (decimal)ds.Tables["RGGSDaten"].Rows[i]["AP_ID"];
                    Artikel_ID = 0;
                    Fracht = (decimal)ds.Tables["RGGSDaten"].Rows[i]["Fracht"];
                    KundeID = (decimal)ds.Tables["RGGSDaten"].Rows[i]["Fracht_ADR"];
                    ...
                    ZusatzKosten = 0.0M;
                    TextZusatzkosten = string.Empty;
                    zwZM = (decimal)ds.Tables["RGGSDaten"].Rows[i]["zw_ZM"];
                    zwAufliefer = (decimal)ds.Tables["RGGSDaten"].Rows[i]["zw_Auflieger"];
                    zwSU = (decimal)ds.Tables["RGGSDaten"].Rows[i]["zw_SU"];
        ...
        private void InsertFrachtRGGS(bool boolGS)
        {
            ...
            try
            {
                ...
                //----- SQL Abfrage -----------------------
                sql = "INSERT INTO Frachten (AP_ID, " +
                                            //"Artikel_ID, " +
                                            "Fracht, " +
                                            "Fracht_ADR, " +
                                            "Frachttext, " +
        ...
        //------------ ID DB Frachten ----------
        //
        private decimal GetIDFromDBbyDaten(bool boolGS)
        {
            Int32 iBit = 0;
            if (boolGS)
            {
        ...
        //
        public DataTable GetManRGGSDaten(decimal KD)
        {
            DataTable dataTable = new DataTable("Auftragsdaten");
            try
            {
        ...
        //
        public static decimal GetAPByFrachtID(decimal FrachtID)
        {
            decimal AP_ID = 0.0m;
            try
            {
